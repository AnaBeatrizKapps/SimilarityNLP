...
vet_ref = nlp.Rmetodoclasse  #vetor referencia a classe
vet_meth = nlp.Vmetodos   #vetor de metodos
vet_class = nlp.Vclasses  #vetor de classes
...
class file_Route:
    def get_file(self):
        ...
        for path_file in paths_file:
            ...
            spec = importlib.machinery.PathFinder().find_spec(module_name,[CAMINHO])
            if not spec:
                continue
...
class ImportFile():
    def import_file(self):
        ...
        #print(name_class)
class ArrangeVet():
#método para separar o vetor do codigo testeNLP
...
class DiceVector():
#colocando na lista os vetores do código testeNLP
    def testeNlp(self):
        a = ArrangeVet()
        vetor_meth = nlp.Vmetodos
        listWord.extend(a.transf_vet(vetor_meth)) #lista de metodos
        #print(listWord)

        vetor_par = nlp.Vparametro 
    ...
    def transf_Map(self,list_Meth1,list_Meth2,list_Par2): #list_Meth1: lista de métodos das classes da pasta cod,listMeth2:lista de métodos do codigo testeNLP, listPar2: lista de parametros do listMeth2
        dices = DiceVector()
        i = -1
        j = -1
        metadados_met = {} 
        ...
        for b in list_Meth2: #adc parâmetros do cod testeNLP
            j = j + 1
...
class DiceMap():     
    ...
    def compareMethods(self,syn1,syn2): #faz a comparação de todos os métodos
        i = 0
        resp = 0
        for a in listFinalMeth:
            calc = wup.similarityWUP(syn1,listFinalMeth[i])
    ...
    def compareParameter(self,syn,metadados,parameters): #faz a comparação do parâmetro passado
        dice = DiceVector()
        i = 0
        j = 0
        resp = 0
        ...
        if dice.isNumber(parameter_syn): 
            for a in parameters:
                calc = wup.similarityWUP(parameter_syn,parameters[i])
                if calc != 1: #se as strings forem iguais não conta, pois chegou no indice em que ela se encontra
                    if calc > resp:
                        resp = calc
                i = i + 1
    ...
    def calc_similarity(self):
        ...
        for syn1 in listWord:
            for syn2 in lista:   
                ...
                if lev > 1: 
                    calc = wup.similarityWUP(listWord[i],lista[j])#percorro as listas e verifico qual é a maior similaridade
                    print(calc)
                    if(calc > resp):
                        resp = calc
                        palavra1 = listWord[i]
                    ...
                    meth = dicesMap.compareMethods(syn1,syn2) #ira comparar todos os metodos com o syn passado
                    par1 = float(dicesMap.compareParameter(syn1,metadados,parameters))
                    par2 = float(dicesMap.compareParameter(syn2,metadados,parameters))
    ...
    def file_class(self):
        ...
        for a in dados.items():
            if(i<conte-1):
                ...
                aux = {'file':'/home/ana/pasta/testeNLP.py','class':a}
                #print(aux)
                map_dados['/home/ana/pasta/testeNLP.py'] = aux
                i = i + 1
